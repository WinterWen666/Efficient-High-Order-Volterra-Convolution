...
import torch.fft
from Volterra_channel import cha_second_conv,cha_third_conv,cha_second_conv_last
...
class volconv(nn.Module):
    def __init__(self, dim, out_dim, order, groups=4):
        super().__init__()
        if order ==1:
            self.conv = nn.Conv2d(dim, out_dim, 1)
        if order ==2:
            self.conv = cha_second_conv(dim, out_dim, groups)
        if order ==3:
            self.conv = cha_third_conv(dim, out_dim, groups)    
...
class H_order_conv(nn.Module):
    def __init__(self, dim,order=2, groups=8, gflayer=None, h=14, w=8, s=1.0):
        ...
        self.proj_in = nn.Conv2d(dim, dim*2,1)
        if gflayer is None:
            self.dwconv = get_dwconv(dim*2, 7, True)
        else:
            self.dwconv = gflayer(dim*2, h=h, w=w)
        self.conv = volconv(dim*2, dim,order, groups)
